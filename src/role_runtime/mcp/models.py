"""MCP tool and server descriptors used by the host object model."""
...
class MCPServer:
    ...
    selects tools. The host uses server_id to route the selected tool to the
    correct MCP client and transport.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if "/" in self.server_id:
            raise ModelValidationError(
                "MCP server id must not contain '/' because tool_ref uses it "
                "as the server/tool delimiter."
            )
