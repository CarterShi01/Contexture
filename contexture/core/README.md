...
```
core/
├── errors.py  types.py  constants.py     共享地基
├── model/            一个能力**是什么**
├── disclosure/       它**在哪**，一次露出**多少**
└── mcp_interface/    在 MCP 三个原语上**暴露什么**
```

**`model/`** —— Role / Skill / Tool 与披露生命周期。节点知道自己叫什么，
不知道自己挂在哪里：位置是树的答案，不是节点的。
...
`errors.py`、`types.py`、`constants.py` 直接放在 `core/` 下：三个子目录都可以
站在它上面，它自己不站在任何东西上面。
